...
describe("подписи для экрана", () => {
  ...
  it("правило словами называет и адресата, и момент, и единственное исключение", () => {
  ...
  // Баг из ledger: текст утверждал, что выключенные напоминания — причина
  // неотправки, а `checklistDelivery` их вовсе не смотрит (единственный
  // молчаливый пропуск — `no-telegram`, см. её комментарий выше). Правило
  // должно называть напоминания как то, что чек-лист переживает, а не как
  // причину, по которой он не уйдёт.
  it("не винит выключенные напоминания в неотправке — чек-лист уходит и так", () => {
    expect(CHECKLIST_RULE_TEXT).not.toMatch(/Не уйдёт[^.]*напоминания/i);
  });
});
