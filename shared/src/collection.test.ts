...
describe("autoSendDateFor", () => {
  ...
  it("на календарно-неверной дате отдаёт сегодня, а не молча перенесённый день", () => {
    // `Date.parse` не отвергает «30 февраля» — он переносит его на 2 марта, и
    // без собственной проверки функция посчитала бы опережение от даты,
    // которой не существует: «сегодня» в этих датах нарочно выбрано РАНЬШЕ
    // перенесённого дня, иначе `max(lead, today)` подменил бы собой проверку
    // и тест остался бы зелёным даже без неё. Тот же вход, что уже отсечён в
    // `formatDayMonth`.
    expect(autoSendDateFor("2026-02-30", "2026-02-20")).toBe("2026-02-20");
    expect(autoSendDateFor("2026-04-31", "2026-04-20")).toBe("2026-04-20");
  });
});
