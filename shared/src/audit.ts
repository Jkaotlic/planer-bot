...
const DESCRIBERS: Record<AuditType, Describer> = {
  ...
  collection_sent: (p) => {
    // `round` — как есть, без подмены отсутствия/нуля на 1: подмена маскировала
    // бы, что раунда в payload не было вовсе, а строка «рассылка №…» и так не
    // рисуется без положительного раунда — подставлять его незачем.
    const round = num(p.round);
    const delivered = num(p.delivered) ?? 0;
    const intended = num(p.intended) ?? 0;
    // `delivered === 0` — это провал доставки (обрыв сети или неверная
    // рассылка), а не «разослан»: «Разослан сбор — доставлено 0 из 2» читался
    // как успех и прятал происшествие в ленте, где его должны заметить.
    const failed = delivered === 0;
    // «0 из 0» не отвечает на вопрос, который у строки возникает первым — а
    // кому вообще пытались отправить: адресатов не было вовсе, это не то же
    // самое, что «были, но не дошло».
    const failureLine = intended === 0 ? "некому было отправлять" : `не дошло ни до кого (0 из ${intended})`;
    return {
      icon: "💰",
      title: failed ? "Сбор не разослан" : round && round > 1 ? "Напоминание о сборе" : "Разослан сбор",
      lines: [
        str(p.title) ?? "сбор",
        ...(round && round > 1 ? [`рассылка №${round}`] : []),
        failed ? failureLine : `доставлено ${delivered} из ${intended}`,
      ],
    };
  },
