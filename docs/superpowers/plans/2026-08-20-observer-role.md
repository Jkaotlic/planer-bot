...
Проверка `expect(assignments).not.toHaveLength(0)` в первом тесте обязательна: без неё пустая раздача (например, если смена не создалась) даёт зелёный тест на любой реализации.
