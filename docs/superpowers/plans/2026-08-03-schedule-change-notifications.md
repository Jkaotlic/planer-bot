...
| 6. сводное письмо и `withScheduleDiff` | ⬜ следующая | — |
...
- [ ] Живая проверка в Chromium на DEV-моке: правка записи в мини-аппе даёт строку про уведомление; «Заполнить неделю» уходит одним запросом (вкладка Network — один `POST /entries/bulk`, а не семь `POST /entries`).
- [ ] Обновить `docs/audit/ledger.md`: фича закрывает дыру, замеченную линзой `notify`.
