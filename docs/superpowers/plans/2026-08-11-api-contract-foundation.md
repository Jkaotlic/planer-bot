...
- **Поведение не меняется.** Эта работа переносит и закрепляет, а не чинит. Любая
  ...
  Исключение — сужение ответов до контракта там, где лишнее поле не читает ни один
  экран: `/api/templates` и `/api/my/shifts` в задаче 2. Оба обоснованы проверкой
  прямо в задаче; второе вскрылось при исполнении, а не при планировании.
...
**Факт исполнения 2026-08-11: упали два случая из трёх, а не один.**

| Ручка | Лишние поля | Разбор |
| --- | --- | --- |
| `/api/templates` | `isActive`, `coverage`, `fillMode`, `rotationUnit`, `primaryEmployeeId` | как и ожидалось — сузить |
| `/api/my/shifts` | `note`, `createdAt`, `updatedAt` | **не было предсказано** — сузить, см. ниже |
| `/api/team/schedule` | — | прошла с первого раза: `readTeamSchedule` уже перечисляет поля явно |

`/api/my/shifts` отдавал весь ряд, потому что `listUpcomingForEmployee` — это
`db.select()` без списка полей. Сужение здесь не меняет поведения и потому сделано
в этой же задаче, а не унесено в ledger:

- `note` — админское свободное поле. Его вырезает `readTeamSchedule` с явным
  комментарием «nobody outside the admin screens should read»; на соседней ручке
  того же домена он уезжал работнику просто потому, что никто не написал список полей.
- Ни `note`, ни `createdAt`, ни `updatedAt` не читает ни один экран: проверено
  `grep -rn "\.note\b|createdAt|updatedAt" miniapp/src admin/src` — все попадания
  `note` относятся к слотам выходного дня (другой домен), а `createdAt` — к обменам,
  журналу и сборам.
- Локальный `Shift` мини-аппа поле `note` пока объявляет; тип перестаёт врать в
  задаче 6, когда он заменяется на `ScheduleEntryDto`.
...
**Правка формы среза, решение владельца 2026-08-11.** План считал мок домена листом,
которому можно отдать свои данные. Это неверно: мок `read` только *читает*
`ALL_ENTRIES` и `EMPLOYEES`, а *пишут* в них около двадцати мест из доменов, которые
ещё не переехали, — создание и правка смены, распределение, импорт ростера, отчёты.
Комментарий в `miniapp/src/api/mock.ts:155` это и оговаривает: правка на экране
администратора обязана быть видна в графике без перезагрузки.

Поэтому:

1. **Состояние — параметр, а не собственность пакета.** `createReadMock({ delayMs, state })`.
   Пакет владеет формой ответа и чтением, фронт — изменяемым состоянием. Мутаторы
   чужих доменов не трогаем, dev-поведение не меняется. Состояние переедет в пакет
   тогда, когда переедет домен `entries`, — и это работа того плана, а не этого.
   `state` необязателен: без него берётся `seedReadMockState()`, сид на трёх
   работниках, ради которого мок запускаем и проверяем сам по себе.
2. **Мок отдаёт чистый DTO.** Записи мини-аппа несут `employeeName`, который он
   приклеивает сам; сервер такого поля не отдаёт, и `.strict()` его отвергает.
   Мок проецирует состояние в DTO по списку полей — тем же способом, что сервер
   в задаче 2. Это заодно убирает расхождение мока с живым путём: `employeeName`
   теперь приклеивается в одном месте, а не запекается в фикстуре.

Для остальных семи доменов это и есть форма: **пакет — форма и чтение, фронт — состояние,**
пока домен, владеющий состоянием, не переехал.

**Ловушка, на которую наступил сид.** Сид привязан к понедельнику текущей недели, а
`getMyShifts` отбрасывает прошедшее. Без записи на последнем дне недели тест «свои
смены не пусты» проходил бы с понедельника по пятницу и падал в субботу и
воскресенье. Проверено прогоном с `vi.setSystemTime` по всем семи дням недели.
Тот же прогон стоит повторять для любого мока, чьи даты считаются от «сегодня».
...
## Что разошлось с планом на задачах 9–10 (2026-08-11)

Форма среза выдержала второй домен без правок — задачи 9–10 делались тем же
способом, что 3 и 5–7. Разошлось другое, и всё найдено исполнением.

**1. Ручек, отдающих ряд таблицы, оказалось пять, а не две.** Задача 8 свела к
`toAdminEmployee` два места, где ряд спредили явно (`{ ...employee }`), и записала
в ledger grep по `\.\.\.employee` как способ искать остальные. Способ неполон:
ещё три ручки отдавали ряд самим объектом — `c.json({ employee })` и
`c.json({ employees })`, — и никакой grep по спреду их не видел:

| Ручка | Что уезжало | Кто читал |
| --- | --- | --- |
| `POST /api/admin/employees` | ряд + `inviteToken` внутри `employee` | только `displayName` и `id` |
| `POST .../:id/role` | тот же ряд | никто, объявлен `Promise<void>` |
| `POST .../:id/order` | ряд **на всю команду** | никто, оба фронта делают `.then(() => {})` |

**Ищущему остальные семь доменов:** грепать надо не спред, а сам факт, что в тело
ответа попадает переменная, полученная из репозитория. Практичнее — контрактный
тест на каждую ручку домена, а не на список.

**2. Расхождение оказалось двусторонним, и вторая сторона важнее.** Все три ручки
не отдавали `address` — вычисляемое поле, которое тип `Employee` обоих фронтов
объявляет `string`. То есть до задачи 10 тип врал про поле, которого в ответе нет;
после реэкспорта `AdminEmployeeDto` он врал бы уже официально. Тот же класс, что
дал `fridayStart` в задаче 6, но замеченный с другого конца: не «сервер отдаёт
`null`, а тип обещает `string`», а «сервер не отдаёт вовсе».

**3. `rosterOrder` в ответе `/order` читал ровно один тест — и это была неправда
про то, что проверяется.** Факт «после перестановки нумерация сплошная» — свойство
колонки, и проверяется теперь через `listForAdmin`. Тест, стоящий на поле ответа,
которого не читает ни один экран, доказывает форму ответа, а не правило.

**4. Транспорт защёлкивал глобальный `fetch` при создании** (`fetchImpl = fetch` в
деструктуризации). Оба фронта создают транспорт при загрузке модуля, а тест
подменяет `fetch` позже — запрос уходил мимо подмены. Поймано только тем, что
`getEmployees` консоли пошёл через пакет: `admin/src/api/offline-message.test.ts`
покраснел. **Поучительнее соседний тест того же файла:** он ждал «нет связи» и
зеленел без подмены вообще — в jsdom настоящий запрос падает сам. Девятая форма
для [[tests-that-cannot-fail]]: **тест, которому окружение даёт тот же результат,
что и починка.**

**5. Мок-тесты фронтов не ускорились** — 14.3 с и 4.4 с против 13.5 и 4.0 до
работы. Не промах: во фронтах задержка мока намеренно 200 мс, выигрыш от нулевой
живёт в тестах пакета (`client/src` — 12 тестов за 0.4 с). Ответ на вопрос 3
раздела ниже: считать выигрыш надо после переезда доменов, которые в этих файлах
и спят, а не после каждого среза.

**6. Схем в домене стало семь вместо четырёх.** Задача 8 описала два списка, но у
домена есть ещё три формы ответа: создание, приглашение, один работник. Для
остальных семи доменов инвентаризацию стоит вести по **ручкам**, а не по
сущностям, — иначе половина форм контракта не получит.
