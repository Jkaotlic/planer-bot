...
export interface TabBarProps {
   ...
   *  Отсутствующий ключ — не ноль, а «нечего показывать»: сама метка внутри
...
function TabIcon({ tab, badges, children }: { tab: TabKey; badges?: Partial<Record<TabKey, number>>; children: ReactNode }) {
  ...
  return (
    <span style={{ position: "relative", display: "inline-flex" }}>
      ...
      {count ? (
        <>
          {/* `aria-hidden`: голый `<span>` без роли `aria-label` не читает ни
              один экранный диктор — число для них должно быть текстом, а не
              атрибутом. Сам кружок остаётся декорацией, `> 9` — «9+», а не
              трёхзначное число: он круглый и тесный, три цифры распирали бы
              его в овал шире соседних букв подписи. */}
