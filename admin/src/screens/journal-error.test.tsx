...
/**
 * «Кто что менял», упавший один раз, не возвращался сам.
 *
 * Ошибка рисовалась ВМЕСТО всего содержимого вкладки — вместе с фильтром
 * событий и кнопками страниц. А эффект перечитывает журнал только когда
 * меняются фильтр или страница, то есть ровно те органы управления, которые
 * ошибка и убрала. Круг замкнут: сервер вернулся, а на экране всё та же
 * «Failed to fetch», и помогает только F5 (или переключение вкладок туда-сюда,
 * если догадаться).
 */
...
describe("«Кто что менял» после отказа возвращается без перезагрузки страницы", () => {
  it("упавший журнал даёт «Повторить», и после него показывает события", async () => {
    ...
    expect(el.querySelector(".journal-row")).not.toBeNull();
  });
});
