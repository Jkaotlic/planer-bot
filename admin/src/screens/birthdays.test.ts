...
import { recipientsPhrase, recipientsSubject, statusOf, whenLabel } from "./BirthdaysScreen";
...
describe("statusOf", () => {
  it("says the link is missing before anything else — it is what blocks sending", () => {
    expect(statusOf(birthday())).toEqual({ label: "Нет ссылки на сбор", tone: "pending" });
    expect(statusOf(birthday({ campaign: campaign() }))).toEqual({ label: "Нет ссылки на сбор", tone: "pending" });
  });
  ...
  it("goes to «готово» once a link is in, without sending anything", () => {
    ...
    expect(statusOf(ready)).toEqual({ label: "Готово к отправке", tone: "ready" });
  });
...
// The Mini App carries its own copy of these four helpers on purpose — it
// depends on neither this console nor its styles. The identical expectations
// live in miniapp/src/screens/admin/birthdays.test.ts; if the two ever
