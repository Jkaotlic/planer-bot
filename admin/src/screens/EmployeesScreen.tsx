      ...
      {!editing && (
        <>
          ...
          {/* Метка только у отписавшихся: она про исключение, а не про норму.
              Привязка тут обязательна — непривязанному не доходит вообще ничего,
              и об этом уже сказал чип слева.

              2026-08-28: трое отписались тапом по 🔕 под сообщением бота и
              месяцами не получали напоминаний, а в консоли выглядели обычными
              работниками. Понять это удалось только запросом к живой базе. */}
          {linked && !employee.remindersEnabled && (
            <span className="status-chip status-chip-alert" title="Человек отписался от вечерних напоминаний. Чек-лист дежурного ему приходит всё равно">
