...
export function createCalendarRoutes(deps: { db: Db; config: Config }): Hono<Env> {
   ...
   * существовал.
   *
   * Токен нигде не логируется этим кодом — но это держится на том, что в
   * процессе нет HTTP-логгера запросов вообще (см. `CLAUDE.md`, «нет логгера
   * доступа»), а не на маскировании пути здесь. Появится access-логгер — этот
   * путь надо будет явно исключить или маскировать в нём, иначе токен
   * окажется в логе первым же запросом.
   *
   * `rateLimiter` считает по TCP-адресу (`clientKey` в `rate-limit.ts`), а не
   * по токену: за облачным релеем KeenDNS все внешние клиенты приходят с
   * одного адреса, так что 60/мин — это общий потолок на всю команду сразу,
   * а не персональный бюджет одного человека.
   */
