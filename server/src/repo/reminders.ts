...
/**
 * День в ключе пометки — только у записи с диапазоном (`endDate > date`): у
 * такой один ряд `shifts` на весь диапазон, и ключ без дня означал бы
 * «чек-лист уже уходил» с первого дня до последнего — инструкция ушла бы один
 * раз на первый день и молчала бы до конца диапазона. Однодневные записи ключ
 * не меняют: тронь его всем — и в день выкатки сегодняшние чек-листы ушли бы
 * второй раз тем, кому уже ушли по старому ключу.
 */
